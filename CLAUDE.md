...
1. **Source plugins** (`src/sources/`) inherit from `SourcePlugin` (abstract `async fetch(limit) → list[Article]`). Five sources: HackerNews (Firebase API), ArXiv (official library), HuggingFace Papers (`/api/papers`), RSS (`rss.py`, a configurable RSS/Atom aggregator driven by `RSS_FEEDS`, used for Chinese sources), and GitHub Trending (`github.py`, parses the static `github.com/trending` + `github.com/explore` pages with BeautifulSoup — no official API; trending score = stars today, explore 推荐仓库 score=0 避免量纲混用，两页按仓库 id 合并去重 trending 优先; `published_at` = collection time since the pages have no dates). All httpx-based sources honor the global proxy: `.env` `HTTPS_PROXY` (empty = direct; system env var takes precedence over `.env` since `load_dotenv` doesn't override). CLI (`src/main.py`) keeps the original three; the scheduler registers all five via `SOURCE_REGISTRY`.
