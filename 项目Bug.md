...
### 机器之心 RSS 免费额度 429 限流（已恢复 + 源头退避）

- **官方守则**: 每个凭证**最多每 60 分钟请求一次，每日最多 25 次**，单次最多返回最近 10 条；超限返回 429，限额恢复后可用
- **现状**: ✅ **2026-08-20 已恢复**（配额重置后 200 返回 10 条真实文章）；✅ 60 分钟退避 + 429 后 6h 冷却已实现（`RSSSource._backoff_active` / `_last_request_ts` / `_last_429_ts` 模块级跨实例共享）——同窗口二次请求必 429（已实证），退避从源头避免
- **影响**: 退避/冷却期间 RSS 源返回空，其他源不受影响（优雅降级）；不是代码问题
...
- **状态**: 已修复
- **发现时间**: 2026-08-20（用户报告"今日摘要"回复中趋势工具连续三次调用失败）
- **修复时间**: 2026-08-20
- **原因**: deepseek-v4-flash 为推理模型，趋势分析（30 篇输入）思考量巨大且不可控——实测 `reasoning_tokens=2048` 全部被思考占满、`finish=length`、`content` 完全为空（0 字符）；tools.py 原 `max_tokens=1200`（analyze_trend）/`1600`（summarize_articles）预算全被 thinking 吃掉 → JSON 解析失败 → LLM 重试 3 次全败
- **解决**: 两处 max_tokens → 4096（推理模型思考防御）；随后模型切换为 qwen-turbo（非推理模型，无思考截断问题），4096 预算保留无副作用
