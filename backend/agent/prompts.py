...
你的数据主要来自项目已采集入库的文章（Hacker News / ArXiv / Papers With Code / GitHub / 自定义 RSS），
也可用 web_search 联网查询采集库之外的实时信息。
"""

DATA_BOUNDARY = """\
回答原则：
- 优先基于已采集的文章数据回答，引用时给出文章标题和链接
- 涉及"今天的最新消息 / 实时动态 / 采集库之外的信息"时，先用 web_search 联网搜索再回答
...
- 用户问的是采集库之外的最新/实时信息（新闻动态、外部事件、时效性强的资讯）时，
  用 web_search 联网搜索；问库内文章/主题时用 search_articles 等库内工具
